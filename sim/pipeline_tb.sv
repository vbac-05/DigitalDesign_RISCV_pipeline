...
task R_type_instruction();
    ...
    begin
        ...
        #(CLOCK_CYCLE);
        @(negedge clk) rst_n = 1;
        $readmemh("/home/vbac/EDABK/Digital_Design/RISCV_pipeline/r32i/R-type/IMEM_hex.txt", DUT.u_imem.imem);
        reset_register_file();
        ...
        $readmemh("/home/vbac/EDABK/Digital_Design/RISCV_pipeline/r32i/R-type/golden_register_file_hex.txt", golden_register_file);
        for (int i = 0; i < 32; i = i + 1) begin
...
task I_type_arithmetic_instruction();
    ...
    begin
        ...
        #(CLOCK_CYCLE);
        @(negedge clk) rst_n = 1;
        $readmemh("/home/vbac/EDABK/Digital_Design/RISCV_pipeline/r32i/I-type(arithmetic&logic)/IMEM_hex.txt", DUT.u_imem.imem);
        reset_register_file();
        ...
        $readmemh("/home/vbac/EDABK/Digital_Design/RISCV_pipeline/r32i/I-type(arithmetic&logic)/golden_register_file_hex.txt", golden_register_file);
        for (int i = 0; i < 32; i = i + 1) begin
            if (DUT.u_pipeline_5stages_core.u_datapath.u_rf.regs[i] !== golden_register_file[i]) begin
                $display("Mismatch at register x%0d: DUT = %h, Golden = %h", i, DUT.u_pipeline_5stages_core.u_datapath.u_rf.regs[i], golden_register_file[i]);
                error_count = error_count + 1;
...
task I_type_load_instruction();
    ...
    begin
        ...
        #(CLOCK_CYCLE);
        @(negedge clk) rst_n = 1;
        $readmemh("/home/vbac/EDABK/Digital_Design/RISCV_pipeline/r32i/I-type(load)/IMEM_hex.txt", DUT.u_imem.imem);
        reset_register_file();
        ...
        $readmemh("/home/vbac/EDABK/Digital_Design/RISCV_pipeline/r32i/I-type(load)/golden_register_file_hex.txt", golden_register_file);
        for (int i = 0; i < 32; i = i + 1) begin
            if (DUT.u_pipeline_5stages_core.u_datapath.u_rf.regs[i] !== golden_register_file[i]) begin
                $display("Mismatch at register x%0d: DUT = %h, Golden = %h", i, DUT.u_pipeline_5stages_core.u_datapath.u_rf.regs[i], golden_register_file[i]);
                error_count = error_count + 1;
...
task S_type_instruction();
    ...
    begin
        ...
        #(CLOCK_CYCLE);
        @(negedge clk) rst_n = 1;
        $readmemh("/home/vbac/EDABK/Digital_Design/RISCV_pipeline/r32i/S-type/IMEM_hex.txt", DUT.u_imem.imem);
        reset_register_file();
        ...
        $readmemh("/home/vbac/EDABK/Digital_Design/RISCV_pipeline/r32i/S-type/golden_register_file_hex.txt", golden_register_file);
        for (int i = 0; i < 32; i = i + 1) begin
            if (DUT.u_pipeline_5stages_core.u_datapath.u_rf.regs[i] !== golden_register_file[i]) begin
                $display("Mismatch at register x%0d: DUT = %h, Golden = %h", i, DUT.u_pipeline_5stages_core.u_datapath.u_rf.regs[i], golden_register_file[i]);
                error_count = error_count + 1;
...
task JAL_instruction();
    ...
    begin
        ...
        #(CLOCK_CYCLE);
        @(negedge clk) rst_n = 1;
        $readmemh("/home/vbac/EDABK/Digital_Design/RISCV_pipeline/r32i/J-type/IMEM_hex.txt", DUT.u_imem.imem);
        reset_register_file();
        ...
        $readmemh("/home/vbac/EDABK/Digital_Design/RISCV_pipeline/r32i/J-type/golden_register_file_hex.txt", golden_register_file);
        for (int i = 0; i < 32; i = i + 1) begin
            if (DUT.u_pipeline_5stages_core.u_datapath.u_rf.regs[i] !== golden_register_file[i]) begin
                $display("Mismatch at register x%0d: DUT = %h, Golden = %h", i, DUT.u_pipeline_5stages_core.u_datapath.u_rf.regs[i], golden_register_file[i]);
                error_count = error_count + 1;
...
task B_type_instruction();
    ...
    begin
        ...
        #(CLOCK_CYCLE);
        @(negedge clk) rst_n = 1;
        $readmemh("/home/vbac/EDABK/Digital_Design/RISCV_pipeline/r32i/B-type/IMEM_hex.txt", DUT.u_imem.imem);
        reset_register_file();
        ...
        $readmemh("/home/vbac/EDABK/Digital_Design/RISCV_pipeline/r32i/B-type/golden_register_file_hex.txt", golden_register_file);
        for (int i = 0; i < 32; i = i + 1) begin
            if (DUT.u_pipeline_5stages_core.u_datapath.u_rf.regs[i] !== golden_register_file[i]) begin
                $display("Mismatch at register x%0d: DUT = %h, Golden = %h", i, DUT.u_pipeline_5stages_core.u_datapath.u_rf.regs[i], golden_register_file[i]);
                error_count = error_count + 1;
...
task JALR_instruction();    
    ...
    begin
        ...
        #(CLOCK_CYCLE);
        @(negedge clk) rst_n = 1;
        $readmemh("/home/vbac/EDABK/Digital_Design/RISCV_pipeline/r32i/I-type(JALR)/IMEM_hex.txt", DUT.u_imem.imem);
        reset_register_file();
        ...
        $readmemh("/home/vbac/EDABK/Digital_Design/RISCV_pipeline/r32i/I-type(JALR)/golden_register_file_hex.txt", golden_register_file);
        for (int i = 0; i < 32; i = i + 1) begin
            if (DUT.u_pipeline_5stages_core.u_datapath.u_rf.regs[i] !== golden_register_file[i]) begin
                $display("Mismatch at register x%0d: DUT = %h, Golden = %h", i, DUT.u_pipeline_5stages_core.u_datapath.u_rf.regs[i], golden_register_file[i]);
                error_count = error_count + 1;
...
task U_type_instruction();
    ...
    begin
        ...
        #(CLOCK_CYCLE);
        @(negedge clk) rst_n = 1;
        $readmemh("/home/vbac/EDABK/Digital_Design/RISCV_pipeline/r32i/U-type/IMEM_hex.txt", DUT.u_imem.imem);
        reset_register_file();
        ...
        $readmemh("/home/vbac/EDABK/Digital_Design/RISCV_pipeline/r32i/U-type/golden_register_file_hex.txt", golden_register_file);
        for (int i = 0; i < 32; i = i + 1) begin
            if (DUT.u_pipeline_5stages_core.u_datapath.u_rf.regs[i] !== golden_register_file[i]) begin
                $display("Mismatch at register x%0d: DUT = %h, Golden = %h", i, DUT.u_pipeline_5stages_core.u_datapath.u_rf.regs[i], golden_register_file[i]);
                error_count = error_count + 1;
